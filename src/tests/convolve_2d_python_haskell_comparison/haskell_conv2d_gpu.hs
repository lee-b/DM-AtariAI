...
main = do
    let img = A.use $ A.fromList (A.Z A.:. (84 :: Int) A.:. (84 :: Int)) [1 | _ <- [1..84*84]]
    let fltr = A.use $ A.fromList (A.Z A.:. (8 :: Int) A.:. (8 :: Int)) [0..63]

    start <- getCurrentTime
    res <- repeateTest 1000 img fltr 4 (0.0)
    stop <- getCurrentTime
    print $ (diffUTCTime stop start) / 1000

--repeateTest :: (Num a, Num b, Eq a) =>
--                              a
...
--                              -> Float
--                              -> IO b
repeateTest counter img fltr strd accum = do
    res <- conv2D (img, fltr, strd)
    let resSum = BE.run (A.sum res)
    if counter == 0 then do
        putStrLn $ show accum
        return 0
    else do
        repeateTest (counter - 1) img fltr strd (accum + A.indexArray resSum (A.Z))
...
conv2D (img, fltr, strd) = do
  ...
  if strd == 2 then do
    ...
    let got = A.stencil (convolveStencil4x4 fltr) (A.Constant 0) img
    ...
    let got = A.stencil (convolveStencil8x8 fltr) (A.Constant 0) img
